...
**Document version:** 3.11 **Date prepared:** July 3, 2026 **Purpose:** This document brings a new contributor (human or AI) fully up to speed on the project so work can continue without losing context.

**Recent changes** (full history in `CHANGELOG.md`):

| Ver | Date | Summary | More |
| --- | --- | --- | --- |
| 3.11 | 2026-07-03 | **Photo Albums (Flickr replacement).** Owner-requested gallery feature outside the roadmap sequence: `photo_albums` + `album_media` (mirrors categories/post_categories); a photo shows in the public gallery only once explicitly assigned to a published album. Editor+ manage albums; any author+ assigns any photo. Cover resolves to an explicit pick or the oldest-assigned photo. Public `/photos` (Flickr-style grid, newest event_date first) + `/photos/{slug}` (paginated grid + lightbox). Admin `/admin/media` gained album filter/search + bulk "Add to album". `Media::paginate()` gained album/search filters. Feature-flagged (`photo_albums`). | §5, §7, §9 |
...
| `photo_albums` | Curated public photo albums (Flickr replacement) — name, slug, event_date, cover, publish state (**added v3.11, migration `0007`**) |
| `album_media` | Junction: albums ↔ media, many-to-many (**added v3.11**) — a photo only appears in the public gallery via this table |
...
Schema migrations live in `settle-private/sql/migrations/`. Seven migrations to date: `0001_add_menu_items.sql`, `0002_add_post_categories.sql` (the `categories` and `post_categories` tables for the v2.0 blog), `0003_add_calendar_hidden.sql` (the `is_hidden` column on `calendar_events_cache` for the v2.3 `[hide]` tag), `0004_add_login_attempts.sql` (the v2.7 login-throttle table), `0005_add_media_thumbnail.sql` (the `thumbnail_filename` column on `media` for the v3.0 thumbnail variant), `0006_add_prayer_chain_optin.sql` (the `allow_prayer_chain` column on `prayer_requests` for the v3.4 opt-in), and `0007_add_photo_albums.sql` (the `photo_albums` + `album_media` tables for the v3.11 gallery feature). **v1.9 added no schema migration** — both calendar tables already existed; the new `google_calendar` config block is config, not schema. Fresh installs run `schema.sql`; existing databases run migrations in order.
...
│   │   ├── AlbumController.php             ← editor+ album CRUD; photo removal (v3.11)
...
│       ├── Album.php                      ← photo album model; cover fallback, bulk assign (v3.11)
...
│   │   ├── albums/{index,edit}.php       ← photo album admin: cover picker + photo grid (v3.11)
...
│       ├── photos_index.php               ← Flickr-style album grid (v3.11)
│       └── photos_album.php               ← paginated photo grid + lightbox (v3.11)
├── bin/
...
* ✅ **Photo Albums (v3.11)** — owner-requested Flickr replacement, outside the roadmap sequence (like Books). New `photo_albums` + `album_media` tables (migration `0007`) mirror the categories/post_categories pattern: a photo appears in the public gallery only once explicitly assigned to a **published** album, so logos/PDFs/misc Media Library files are never auto-included. Editor+ manage albums (create/rename/publish/delete, cover picker reusing the slideshow's `media_id` picker pattern, `event_date` for the Flickr-style "Aug 2025" label and default newest-first sort); **any author+ may assign any photo to any album** (owner decision — not restricted to the assigner's own uploads, since albums span multiple event photographers). Cover resolves to an explicit pick, else the oldest-assigned photo, else a plain placeholder. `/admin/media` gained an album filter, a caption/filename search box, and a multi-select "Add to album" bulk action; the single-photo edit page gained an album checkbox fieldset. `Media::paginate()` gained optional `album_id`/`search` filters. Public **`/photos`** (album grid) + **`/photos/{slug}`** (paginated 40/page grid + dependency-free lightbox). Feature-flagged (`photo_albums`); `/photos` in the menu URL picker. `AlbumController`/`Album` model; 53-assertion SQLite + render harness. **Addendum:** the admin help doc (`\Settle\Help`, #14) gained a matching `photo-albums` section + matrix row and a cross-linked update to the existing `photos` section, 19-assertion harness.
