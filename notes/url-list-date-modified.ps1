...
$urls = Get-ChildItem -Recurse -File | 
    ...
    ForEach-Object {
        ...
        $fileUrl = "$repoUrlBase/$urlPath"

        # Get and format the file's last modified date
        $fileDate = $_.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss")

        # Get the file size in bytes
        $fileSize = $_.Length

        # Output format: Date | Size (Bytes) | URL
        "$fileDate | $fileSize bytes | $fileUrl"
    }

# 5. Save to file
$urls | Out-File notes\urls-details.txt

Write-Host "Done! Processed $(($urls).Count) files and saved their details to urls-details.txt" -ForegroundColor Green